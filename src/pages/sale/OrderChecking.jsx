...
const OrderChecking = () => {
  ...
  useEffect(() => {
    ...
    const getBoxOption = async (boxOptionId) => {
      ...
      if (resOfBoxOption) {
        let matchedBoxOption = resOfBoxOption.data.data.find(
          (option) => option.boxOptionId === boxOptionId
        );
        if (matchedBoxOption) {
          setBoxOption((prevOptions) => [...prevOptions, matchedBoxOption]);
          if (
            matchedBoxOption.fishes &&
            Array.isArray(matchedBoxOption.fishes)
          ) {
  ...
  const totalQuantity = boxOption.reduce((total, option) => {
    const fishQuantities = option.fishes.reduce(
      (sum, fish) => sum + fish.quantity,
      0
    );
  ...
  return (
    <MainCard>
      <Typography variant="h4" align="left" gutterBottom>
        Trạng thái đơn hàng:{" "}
        {statusMessages[order.isShipping] || "Không xác định"}
      </Typography>

      <Box
        sx={{
          backgroundColor: statusColors[order.isShipping],
          padding: 2,
          borderRadius: 1,
          marginBottom: 2,
        }}
      ...
      >
        ...
        <Paper sx={{ padding: 2, flex: 1, margin: 1, textAlign: "start" }}>
          ...
          <Box display="flex" justifyContent="space-around" marginTop={2}>
            {[order.urlCer1, order.urlCer2, order.urlCer3, order.urlCer4].map(
              (url, index) => (
                <Paper
                  key={index}
                  sx={{
                ...
                <TableRow key={boxOption.boxOptionId}>
                  ...
                  <TableCell align="right">
                    {boxOption.fishes.map((fish) => (
                      <Box key={fish.fishId}>
                        {fish.quantity}x {fish.fishDescription} ({fish.fishSize}{" "}
                        cm)
                      </Box>
                    ))}
                  </TableCell>
          ...
          <Table>
            ...
            <TableBody>
              <TableRow>
                ...
                <TableCell align="right">
                  {boxOption.some((box) => box.boxName.includes("-JP"))
                    ? boxOption
                        .filter((box) => box.boxName.includes("-JP"))
                        .reduce((total, box) => total + box.price, 0)
                        .toLocaleString() + " VND"
                    : "0 VND"}
                </TableCell>
                <TableCell align="right">
                  {
                    boxOption.filter((box) => box.boxName.includes("-JP"))
                      .length
                  }{" "}
                  hộp
                </TableCell>
              ...
              {/* Thêm phần hiển thị chi phí vận chuyển nội địa từ orderDetail */}
              {currentOrderDetail.map((detail) => (
                <TableRow key={detail.id}>
                  <TableCell>Chi phí vận chuyển trong nước</TableCell>
                  <TableCell align="right">
                    {detail.totalShippingFee.toLocaleString()} VND
                  </TableCell>
                  <TableCell align="right">Hộp Option ID: </TableCell>
                </TableRow>
              ))}
                ...
                <TableCell align="right">
                  <strong>
                    {(
                      ...
                      currentOrderDetail.reduce(
                        (total, detail) => total + detail.totalShippingFee,
                        0
                      )
                    ).toLocaleString()}{" "}
                    VND
      ...
      <Dialog open={openDialog} onClose={handleCloseDialog}>
        ...
        <DialogContent>
          <Typography>
            ...
            {dialogAction === "accept" ? "xác nhận" : "từ chối"} đơn hàng này?
          </Typography>
        </DialogContent>
        <DialogActions>
